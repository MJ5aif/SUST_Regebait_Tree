...
\[ S_j = \sum_{1 \le i_1 < \dots < i_j \le n} |A_{i_1} \cap \dots \cap A_{i_j}| \]

\begin{itemize}
    \item \textbf{Union (At least 1):} \\
    $|A_1 \cup \dots \cup A_n| = \sum_{j=1}^n (-1)^{j-1} S_j$
    
    \item \textbf{Exactly $k$ properties:} \\
    $E_k = \sum_{j=k}^n (-1)^{j-k} \binom{j}{k} S_j$
    
    \item \textbf{At least $k$ properties:} \\
...
\begin{enumerate}[resume]
    ...
    \item Number of permutations of $\{1, \dots, n\}$ that avoid the pattern $123$ (or any of the other patterns of length $3$); that is, the number of permutations with no three-term increasing subsequence. For $n = 3$, these permutations are $132, 213, 231, 312$ and $321$. For $n = 4$, they are
          $1432, 2143, 2413, 2431, 3142, 3214,$ \newline $3241, 3412, 3421, 4132, 4213, 4231, 4312$ and $4321.$
...
\begin{enumerate}[resume]
  ...
  \item Continuing from previous point, \\
      {\raggedright $\displaystyle \left(\frac{p}{5}\right) = $ infinite concatenation of the sequence \\
      $(1,-1,-1,1,0)$ from $p \geq 1$. \par}
